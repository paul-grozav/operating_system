...
void detect_pci_devices()
{
  ...
  for (uint8_t bus = 0; bus <= 255; bus++)
  {
    for (uint8_t device = 0; device < 32; device++)
    {
      ...
      uint32_t tmp_oc = pci_read(bus, device, 0, 0xc);
      ...
      for (uint8_t function = 0; function < no_functions; function++)
      {
        if ((pci_read(bus, device, function, 0) & 0xffff) != 0xffff)
        {
          ...
          uint32_t tmp_o4 = pci_read(bus, device, 0, 4);
          ...
          module_terminal_global_print_c_string("Detected PCI device: vendor ");
          ...
          module_terminal_global_print_uint64(di.command);
